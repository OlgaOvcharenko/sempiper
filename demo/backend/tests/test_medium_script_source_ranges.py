...
def test_var_nodes_have_correct_lines_v1():
    ...
    # Line 4: products = skrub.var("products", None)
    ...
    assert products_var[0].source_range.start_line == 4, \
        f"products var should start at line 4, got {products_var[0].source_range.start_line}"

    # Line 5: baskets = skrub.var("baskets", None)
    ...
    assert baskets_var[0].source_range.start_line == 5, \
        f"baskets var should start at line 5, got {baskets_var[0].source_range.start_line}"


def test_subsample_node_line_v1():
    """Subsample should map to line 6 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 6: baskets = baskets.skb.subsample(n=100)
    ...
    assert subsample_nodes[0].source_range.start_line == 6, \
        f"subsample should start at line 6, got {subsample_nodes[0].source_range.start_line}"


def test_getitem_for_as_x_maps_correctly_v1():
    """GetItem node for baskets[["ID"]] should map to line 7 (same as as_X) in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 7: basket_ids = sempipes.as_X(baskets[["ID"]], "X")
    ...
    assert first_getitem.source_range.start_line == 7, \
        f"First GetItem (for as_X) should map to line 7, got {first_getitem.source_range.start_line}"


def test_getitem_for_as_y_maps_correctly_v1():
    """GetItem node for baskets["fraud_flag"] should map to line 8 (same as as_y) in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 8: fraud_flags = sempipes.as_y(baskets["fraud_flag"], "y")
    ...
    assert fraud_flag_getitem.source_range.start_line == 8, \
        f"fraud_flag GetItem should map to line 8, got {fraud_flag_getitem.source_range.start_line}"


def test_sem_fillna_node_line_v1():
    """sem_fillna node should map to line 9 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 9: products = products.sem_fillna(target_column="make", nl_prompt="Infer", impute_with_existing_values_only=True)
    ...
    assert fillna_nodes[0].source_range.start_line == 9, \
        f"sem_fillna should start at line 9, got {fillna_nodes[0].source_range.start_line}"
...
def test_sem_gen_features_node_line_v1():
    """sem_gen_features node should map to line 11 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 11: kept_products = kept_products.sem_gen_features(nl_prompt="Generate", name="features", how_many=3)
    ...
    assert gen_features_nodes[0].source_range.start_line == 11, \
        f"sem_gen_features should start at line 11, got {gen_features_nodes[0].source_range.start_line}"


def test_skb_apply_node_line_v1():
    """skb.apply should map to line 13 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 13: vectorized = kept_products.skb.apply(vectorizer, exclude_cols=["basket_ID"])
    ...
    assert skb_apply_nodes[0].source_range.start_line == 13, \
        f"skb.apply should map to line 13, got {skb_apply_nodes[0].source_range.start_line}"


def test_groupby_node_line_v1():
    """groupby node should map to line 14 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 14: aggregated = vectorized.groupby("basket_ID").agg("mean").reset_index()
    ...
    assert groupby_nodes[0].source_range.start_line == 14, \
        f"groupby should map to line 14, got {groupby_nodes[0].source_range.start_line}"


def test_merge_node_line_v1():
    """merge node should map to line 15 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 15: augmented = basket_ids.merge(aggregated, left_on="ID", right_on="basket_ID").drop(columns=["ID", "basket_ID"])
    ...
    assert merge_nodes[0].source_range.start_line == 15, \
        f"merge should start at line 15, got {merge_nodes[0].source_range.start_line}"


def test_drop_node_line_v1():
    """drop node should map to line 15 (same line as merge) in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 15: ... .drop(columns=["ID", "basket_ID"])
    ...
    assert drop_nodes[0].source_range.start_line == 15, \
        f"drop should map to line 15, got {drop_nodes[0].source_range.start_line}"


def test_apply_with_sem_choose_node_line_v1():
    """apply_with_sem_choose node should map to line 16 in v1."""
    result = compile_script_to_graph_dynamic(MEDIUM_SCRIPT_V1)

    # Line 16: clf = augmented.skb.apply_with_sem_choose(HistGradientBoostingClassifier(), y=fraud_flags, choices=sempipes.sem_choose(name="c"))
    ...
    assert apply_choose_nodes[0].source_range.start_line == 16, \
        f"apply_with_sem_choose should start at line 16, got {apply_choose_nodes[0].source_range.start_line}"
...
def test_multiline_sem_fillna_v2():
    ...
    # Lines 10-12: products = products.sem_fillna(...)
    fillna_nodes = [n for n in result.nodes if n.label == "sem_fillna"]
    assert len(fillna_nodes) == 1, f"Should find exactly one sem_fillna node"
    assert fillna_nodes[0].source_range is not None, "sem_fillna should have source_range"
    assert fillna_nodes[0].source_range.start_line == 10, \
        f"Multiline sem_fillna should start at line 10, got {fillna_nodes[0].source_range.start_line}"
...
def test_multiline_sem_gen_features_v2():
    ...
    # Lines 15-18: kept_products = kept_products.sem_gen_features(...)
    gen_features_nodes = [n for n in result.nodes if n.label == "sem_gen_features"]
    assert len(gen_features_nodes) == 1, f"Should find exactly one sem_gen_features node"
    assert gen_features_nodes[0].source_range is not None, "sem_gen_features should have source_range"
    assert gen_features_nodes[0].source_range.start_line == 15, \
        f"Multiline sem_gen_features should start at line 15, got {gen_features_nodes[0].source_range.start_line}"
...
def test_different_variable_names_v3():
    ...
    assert prods_var[0].source_range.start_line == 4, \
        f"prods var should start at line 4, got {prods_var[0].source_range.start_line}"
    ...
    assert orders_var[0].source_range.start_line == 5, \
        f"orders var should start at line 5, got {orders_var[0].source_range.start_line}"
