...
test.describe("Full-stack demo", () => {
  test("page loads and shows Run button and computation graph", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("button", { name: /run/i })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Computation graph" })).toBeVisible();
    await expect(page.getByText(/Computation graph \(from code\)/i)).toBeVisible({ timeout: 10000 });
  });
