...
export function CodeOutput({ code, language, isLoading }: CodeOutputProps) {
  ...
  useEffect(() => {
    ...
    codeToHtml(code, {
      ...
      .catch(() => {
        if (!cancelled) {
          setHtml(`<pre class="p-4 text-slate-300 font-mono text-sm">${escapeHtml(code)}</pre>`);
          setLineCount((code.match(/\n/g) ?? []).length + 1);
        }
      });
    return () => {
  ...
  if (isLoading) {
    return (
      <div className="h-full w-full rounded-lg border border-slate-700 bg-slate-900/80 flex items-center justify-center">
  ...
  if (!useVirtual) {
    return (
      <div className="h-full w-full rounded-lg border border-slate-700 bg-slate-900 overflow-auto">
        <div
  ...
  return (
    <div
      ref={parentRef}
      className="h-full w-full rounded-lg border border-slate-700 bg-slate-900 overflow-auto"
    >
      <div
      ...
      >
        {virtualItems.map((item) => (
          <div
            ...
            className="flex items-center px-4 text-slate-300 font-mono text-sm"
          >
            {code.split("\n")[item.index]}
          </div>
        ))}
